...
def sender(browser,message):  #this function sends the whatsapp message
    import time
    ...
    button=browser.find_element_by_css_selector('#main > footer > div._2i7Ej.copyable-area > div:nth-child(3) > button')
    #button=browser.find_element_by_css_selector('#main > footer > div._2i7Ej.copyable-area > div:nth-child(3) > button > span')
    button.click()
def funcamzn(browser,res):  #scrapes final list of products
    import bs4
    soup= bs4.BeautifulSoup(res.text,'html.parser')
    elem = soup.findAll("span",{"id":"productTitle","class":"a-size-large"})
    elem2 = soup.findAll("span",{"id":"priceblock_ourprice"})
    try:
        sender(browser,str(elem[0].text.strip())+" Cost:"+str(elem2[0].text.strip()))
    except:
...
def amznurl(browser,item): #forms list of products
    ...
    for i in range(1,90):
        ...
        mo=obj.findall(x)
        if(len(mo)>0):
                    final.append('https://www.amazon.in'+mo[0].strip('href="').strip('#customerReviews'))#final list of items
...
def translator(browser,str1):
    import bs4,time
    from selenium import webdriver
    bro=webdriver.Chrome()
    bro.get('https://translate.google.com/#view=home&op=translate&sl=auto&tl=hi&text='+str1)
    time.sleep(1)
    #chrome.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
    try:
        msg2=bro.find_element_by_css_selector('body > div.frame > div.page.tlid-homepage.homepage.translate-text > div.homepage-content-wrap > div.tlid-source-target.main-header > div.source-target-row > div.tlid-results-container.results-container > div.tlid-result.result-dict-wrapper > div.result.tlid-copy-target')
...
def temp(browser,city):
    import requests,bs4
    try:
        res=requests.get('https://www.timeanddate.com/weather/india/'+city+'/ext')
    except:
...
user.click()
lis=[]
useless_dict={'hello':'hi human ','hi':'hello!!! human','how are you':'fine and working','how are you ?':'fine and working',
              'hello,how are you ?':'i am fine and working' }

while(True):
    lis.clear()
    time.sleep(2)
    exp=browser.find_elements_by_xpath('//span[@dir="ltr"][@class="selectable-text invisible-space copyable-text"]')
